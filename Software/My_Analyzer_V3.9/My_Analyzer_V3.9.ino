 ...
 *
 *		Fixed a bug! When doing repeated scans, the minimum SWR and frequency wasn't
 *		displaying at the top of the screen after each scan. 
 *
 *		Also added the ability to set the delay time between scans.
 *
 *
 ...
 *	reason you need to use the display upside down, change the "LANDSCAPE" definition to "3".
 */
...
volatile uint8_t 	encoderDirection;		// Which way the encoder was moved
volatile int16_t	clickCount;				// How many clicks was it moved
...
/*
 *	Definitions for DDS type (DON'T CHANGE THESE):
 */
...
volatile int16_t	FT_pushCount     = 0;		// Ranges from 0 to 3 (but sometimes 4)
volatile int16_t	calPushCount     = 0;		// Ranges from 0 to 3
volatile uint32_t	FT_Time 		 = 0;		// Time the button was first pushed
...
int16_t		freqIncrements[4] = { 100, 10, 1, 1000 };	// The active increment factors
int16_t		activeFreqIncrement;						// The currently selected one

int16_t		calIncrements[4]  = { 1000, 100, 10, 1 };	// Used in calibration function
int16_t		activeCalIncrement;							// The currently selected one
 ...
 *
 *	They are defined as integers as opposed to using #define statements. The code in "setup"
 *	inverts them based on some display types. I may change this in the future.
 */
...
int16_t DKGREEN = 0x03E0;
int16_t DKGREY	= 0x8410;		// Used on SWR meter & scan plots
int16_t MAGENTA = 0xF81F;		// Only used on first 2 lines of the startup screen

int16_t ORANGE  = 0xFA60;		// Not used
int16_t LIME    = 0x07FF;		// Not used
int16_t AQUA    = 0x04FF;		// Not used
int16_t LTPINK  = 0xFDDF;		// Not used
int16_t LTGREY  = ~0xE71C;		// Not used
int16_t	PINKY    = 0xF8FF;		// Not used - Compiler does not like the name "PINK"!
...
int16_t		menuIndex;						// Index to currently selected main menu item
int16_t		subMenuIndex;					// Index to the currently selected item in a sub-menu
int16_t		nextSD_FileNumber;				// Next number to use in file naming (stored in EEPROM)

int16_t		fileCount = 0;					// Number of files read from card (limited by MAX_FILES)
int16_t		useFile	  = 0;					// Index of a selected file

int16_t		menuColumns[3] = { 0, 12, 25 };	// Columns for main & sub menus
int16_t		menuRow;						// Row and columns for menu displays 
int16_t		menuCol;						// (text size values not pixels)

int16_t		displayW;						// Width & Height of display
int16_t		displayH;

uint16_t 	g_identifier;					// Identifier of the TFT display type

int16_t		activeBandSet;					// Valid band data indicator
int16_t		activeBandIndex;				// From EEPROM active band for scans
...
uint32_t	baudRates [] = { 300, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 74880, 115200 };

uint32_t	baudRate = 115200;			// Default 115200 baud
 ...
 */
	...
	int16_t	calFactor;					// Fudge factor in Edwin's SWR formula
#endif
...
int16_t	bandCount = 10;								// 10 Without the optional bands
...
{
	const int16_t	xOrigin     =  60;		// Upper left horizontal coordinate
	const int16_t	yOrigin     = 290;		// Lower left vertical coordinate
	const int16_t	graphWidth  = 350;		// Width of graph
	const int16_t	graphHeight = 260;		// Height of graph
	...
	int16_t			minIndex;				// Array index of minimum SWR/frequency pair
...
struct barStruct							// Graph structure declaration for bar graphs
{
	const int16_t	xOrigin     =  60;		// Upper left horizontal coordinate
	const int16_t	yOrigin     = 290;		// Lower left vertical coordinate
	const int16_t	graphWidth  = 410;		// Width of graph
	const int16_t	graphHeight = 260;		// Height of graph

	const int16_t	Width	    =  24;		// Width of a bar
	const int16_t	Space 	    =  10;		// Space between bars
	int16_t			X;						// Horizontal position of current bar
...
struct meterStruct
{
	const int16_t	startX	  = 50;				// Starting X coordinate (upper left hand corner)
	const int16_t	startY	  =  5;				// Starting Y coordinate

	const int16_t	frameW   = 380;				// Width of outer frame
	const int16_t	frameH   = 100;				// Height of outer frame

	const int16_t	scaleX   =  80;				// Left side of scale line
	const int16_t	scaleY   =  startY + 50;	// Vertical position of scale line
	const int16_t	scaleLen = 320;				// Length of scale line
...
int16_t		repeatCount = DEFAULT_COUNT;		// How many times to repeat the scan
int16_t		scanCount = 0;						// How many we've done
uint32_t	scanPause = SCAN_PAUSE;				// Delay between repeated scans
 ...
 *
 *	Since this was originally written, we've discovered that there are other display types
 *	that require inversion. If you have one of these, simply change one of the values in
 *	the "if" statement to the type display you have.
 */
...
void loop()
{
	int16_t oldIndex;						// Saves & restores the menu item selected
...
void SetBandEdge ( int16_t whichOne )
{
...
uint16_t GetBandEdge ( int16_t bandIndex, int16_t whichOne )
{
	if ( whichOne == LOWER )
...
uint16_t GetActiveEdge ( int16_t whichOne )
{
	if ( whichOne == LOWER )
 ...
 */
	...
	int16_t val = scan.minSWR * 100.0;					// Integer version of SWR
 ...
 *	Modified in Version 3.9:
 *
 *		If the encoder switch is pressed to stop repeating, the current scan will be
 *		completed. In previous versions, scanning stopped as soon as the switch was operated.
 *
 *		Added the ability for the user to set the time between scans.
 */
...
/*
 *	Allow the user to set the delay time between scans. The initial value displayed will
 *	be the value of the "SCAN_PAUSE" symbol defined in "My_Analyzer.h". "MAX_SCAN_PAUSE"
 *	can also be changed in "My_Analyzer.h".
 */

	tft.fillScreen ( BLACK );						// Fade to black
	itoa ( scanPause, buff, 10 );					// Make delay time ASCII
	
	PaintText ( 3, SCREEN_C,  2, 0, NORMAL,  CENTER, "Set delay time:" );
	PaintText ( 3, SCREEN_C,  4, 0, HEADING, CENTER, buff );
	PaintText ( 2, SCREEN_C, 10, 0, NORMAL,  CENTER, msgPushButton );
	...
	while ( digitalRead ( SWITCH ))					// While encoder switch not pushed
	{
		if ( encoderDirection != DIR_NONE )			// If the encoder moved
		{
			...
			{
				case DIR_CW:							// Next higher
					scanPause += 1;						// Increment the delay time
					if ( scanPause > MAX_SCAN_PAUSE )	// Upper limit?
						scanPause  = MAX_SCAN_PAUSE;
					break;

				case DIR_CCW:							// Next lower
					scanPause -= 1;						// Decrement the delay time
					if ( scanPause < 1 )				// Out of bounds
						 scanPause = 1;
			...
			itoa ( scanPause, buff, 10 );					// Make repeat count ASCII
	...
	for ( int i = 0; i < repeatCount; i++ )
	{
		...
		delay ( scanPause * 1000 );					// Time between scans
	}
}
...
void Examine ()
{
	...
	int16_t		index;;								// Index to frequency/SWR pair
	
	int16_t		xPos;								// X coordinate of current vertical index line
	int16_t		lastX;								// X coordinate of previous vertical index line
	
	int16_t		yPos;								// X coordinate of current horizontal index line
	int16_t		lastY;								// X coordinate of previous horizontal index line
...
/*
 *	Take a few readings while the frequency settles in!
 */
 ...
 */
	...
	while ( digitalRead ( SWITCH ))						// While encoder switch not operated
	{
		...
		delay ( 250 );							// 1/4 second between readings
	}											// End of "while ( digitalRead ( SWITCH ))"
...
void DrawBarChart ()
{
	int16_t	horizLines[]  = { 0, 38, 78, 124, 182, 220, 260 };			// Blue line Y coordinates
	int16_t	greyLines[]   = { 21, 53, 66, 103, 157, 202, 235, 248 };	// Grey line Y coordinates
	
	char* 	labelsY[] = { "1.00", "1.40", "2.00", "3.00", "5.00", "7.00", "10.0" };

	int16_t	lineY;							// Current Y coordinate
	int16_t	color;							// Bar color
...
void DoSetOptions ()
{
	...
	int16_t		valueCount = ELEMENTS ( menuOptions );	// Count of values
	int16_t		valueCol = 27;							// Column of menu values
	int16_t		oldColumn = menuCol;					// Column of currently selected item
	int16_t		oldIndex  = menuIndex;					// Index of currently selected item
	int16_t		row;									// Row of newly selected item
...
/*
 *	Restart the serial monitor at the new baud rate:
 */
 ...
 *
 *	I have since discovered "sprintf" does not support all the formatting options on the
 *	Arduino that are available on real machines!
 */
...
void Splash ()
{
	...
	PaintText ( 2, SCREEN_C,  5, 0, ERR,     CENTER, "Upgraded Software Version 3.9" );
...
{
	int16_t		MHz;							// MHz part of frequency
	int16_t		KHz;							// Entire KHz part

	int16_t		KHz_Digit_1;					// Going to break KHz into 3 digits
	int16_t		KHz_Digit_2;
	int16_t		KHz_Digit_3;

	int16_t		Digit = 0;						// Where are we in the buffer
...
{
	int16_t	Integer;							// Integer part of SWR
	int16_t	Decimal;							// Entire fractional part

	int16_t	Dec_Digit_1;						// Going to break decimal part into 2 digits
	int16_t	Dec_Digit_2;
...
{
int16_t	fontWidth  = sz * SZ_1_W;					// Width of a single character in specified size
int16_t	fontHeight = ( sz + spacing ) * SZ_1_H;		// Height of a single character + spacing

int16_t	maxColumn  = displayW / fontWidth;			// Maximum number of columns based on character size
int16_t	maxRow     = displayH / fontHeight;			// Naximum number of rows

int16_t	horiz;										// Horizontal cursor address (pixel column)
int16_t	vert;										// Vertical cursor address (pixel row)

int16_t	stringPixels;								// String length in pixels
...
int16_t	stringPixels;						// Fill length in pixels
...
void GraphAxis ()
{
	...
	int16_t 	vertLines[]   = { 60, 130, 200, 270, 340, 410 };	// X-axis vertical line locations
	int16_t 	horizLines[]  = { 0, 38, 78, 124, 182, 220, 260 };	// Y-axiz horizontal line locations
	int16_t		greyLines[]   = { 21, 53, 66, 103, 157, 202, 235, 248 };	// Intermediate lines
...
{
	int16_t	pointerTop = meter.scaleY + 4;				// Y coordnate of the top of the pointer
	int16_t	pointerW   = 3;								// Pointer width
	int16_t	pointerL   = 37;							// Length of pointer
...
float ReadSWRValue ( bool doingScan )
{
    ...
    if ( optStats && optStatsLogIndiv )	// divide the output listing for readability
	{
		Serial.println(F(" "));
		Serial.println(F("========"));
	}
    ...
    for ( i = 0; i < samples; i++ )			// Take multiple samples at each frequency
	{
		...
		if ( optStats )
		{
			...
			if ( optStats && optStatsLogIndiv )
			{
				...
				if ( !digitalRead ( SWITCH ))				// Return if button is being pressed
				{
					stopRepeating = true;
					return 0.0;
				}      
...
void ShowFiles()
{
	int16_t	k;								// Loop index
	char	buff[15];						// For file name

	int16_t firstRow   = 2;					// First row to display files on
	int16_t	displayRow = firstRow;			// Row position of 1st file name
	int16_t	displayColumn;					// Column for file name
